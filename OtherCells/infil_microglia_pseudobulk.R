...
load('infil_microglia_pseudobulk.Rdata')
...
save(cluster_counts,pbmc3,cluster_metadata,dds,res,res_tbl,sig_res,
     ...
     file='infil_microglia_pseudobulk.Rdata')
...
##筛选microglia
cluster_metadata <- metadata[which(metadata$sccelltype == 'Microglial_Cell'), ]
...
gmtfile = read.gmt('c7.all.v2022.1.Hs.symbols.gmt')    #免疫特征基因集    #4

gmtfile = read.gmt('GOBP_MICROGLIA_DIFFERENTIATION.v2022.1.Hs.gmt')  #1
gmtfile = read.gmt('KEGG_GLIOMA.v2022.1.Hs.gmt')   #0
