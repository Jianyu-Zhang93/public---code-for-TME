...
load('infil_immune_pseudobulk.Rdata')
...
save(cluster_counts,pbmc3,cluster_metadata,dds,res,res_tbl,sig_res,
     ...
     file='infil_mi_ma_pseudobulk.Rdata')



load('infil_mi_ma_pseudobulk.Rdata')
...
##筛选
cluster_metadata <- metadata[which(metadata$`sccelltype` %in% c('Microglial_Cell','Macrophage')), ]
...
write.csv(sig_res,
          ...
          file = "0.05_macrophage_genes.csv",
          quote = FALSE, 
          row.names = FALSE)
...
ei_meta = cluster_metadata[, c("characteristics:.cell.type", "tissue" )]
