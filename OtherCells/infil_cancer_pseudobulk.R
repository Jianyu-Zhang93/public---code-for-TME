...
load('infil_cancer_pseudobulk.Rdata')
...
save(cancer_qcdata,pbmc3,cancermetadata,dds,res,res_tbl,sig_res,
     gathered_top20_sig,ei_meta,   
     
     file='infil_cancer_pseudobulk.Rdata')
...
# Plot heatmap
pheatmap(rld_cor, annotation = cancermetadata[, c("tissue"), drop=F])
...
levels(cancermetadata$tissue)[2] = 'Tumor'
levels(cancermetadata$tissue)[1] = 'Periphery'

contrast <- c("tissue", levels(cancermetadata$tissue)[2], levels(cancermetadata$tissue)[1])
...
ei_meta = cancermetadata[, c("sccelltype", "tissue" )]
