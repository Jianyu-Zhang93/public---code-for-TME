...
core_means <- read.delim(paste0(core,"means.txt"), check.names = FALSE)

peri_pvals <- read.delim(paste0(peri,"pvalues.txt"), check.names = FALSE)
peri_means <- read.delim(paste0(peri,"means.txt"), check.names = FALSE)
...
pldf%>% filter(means >0) %>% 
  ...
  scale_color_gradient2(high="red",mid = "yellow",low ="darkblue",midpoint = 0.3  )+ theme_bw()+ 
